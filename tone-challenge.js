...
(function () {
  ...
  /* ----------------------------------------------------------------------
     ...
     feedback by glowing the frames of BOTH the word card and the pitch-graph box
     in the tier's colour (no scale/expand — same glow language as the other game
     modes, just five colours). The glow fades in on score and clears on the next
     word. This leaves the graph interior clean so the pitch contour stays easy
     to read.
  ...
  function tcFbStyle() {
    ...
    var css =
      // Smooth fade for the glow on both frames.
      '.tc-word-card,#view-tone-challenge .tc-graph-wrap{' +
        'transition:box-shadow 260ms ease,border-color 260ms ease;}';
    for (var i = 0; i < TIERS.length; i++) {
      var name = TIERS[i][0], rgb = TIERS[i][1], shadow = TIERS[i][2];
      css +=
        '.tc-word-card.tc-fb-' + name + ',' +
    ...
    css += '@media (prefers-reduced-motion: reduce){' +
      '.tc-word-card,#view-tone-challenge .tc-graph-wrap{transition:none;}}';
  ...
  function tcFbTargets() {
    return [$('tc-word-card'), tcGraphWrap()];
  }
